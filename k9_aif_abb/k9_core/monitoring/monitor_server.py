...
class MonitorServer(BaseMonitor):
    ...
    -------------------------------------
    Central aggregator for all monitoring backends.
    Fan-outs metrics and events to every registered monitor
    (e.g., LoggerMonitor, ConsoleMonitor, CloudWatchMonitor).

    Responsibilities:
    - Broadcasts telemetry across 1..N monitor instances.
    - Adds shared `common_tags` for consistent observability metadata.
    - Ensures observability never disrupts functional flow.
    """
