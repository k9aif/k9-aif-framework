...
class RetrieverAgent(BaseAgent):
    """SBB for hybrid knowledge retrieval — combines VectorDB, keyword, and router-based LLM reasoning."""
    ...
    async def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Hybrid retrieval entrypoint (vector, keyword, or router-based LLM reasoning)."""
        ...
        self.logger.info(
            f"[{self.layer}] Searching collection={collection_name}, query='{query}', top_k={top_k}"
        )
        ...
        if (not results or len(results) == 0) and self.router:
            ...
            try:
                ...
                self.logger.info(
                    f"[{self.layer}] Router fallback complete "
                    f"(model={response.model_alias}, provider={response.provider})."
                )

            except Exception as le:
                self.logger.error(f"[{self.layer}] Router fallback failed: {le}")
                traceback.print_exc()
