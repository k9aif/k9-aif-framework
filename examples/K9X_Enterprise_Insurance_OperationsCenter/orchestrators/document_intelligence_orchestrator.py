...
# K9-AIF EOC — DocumentIntelligenceOrchestrator (SBB)
#
# Subscribes to: eoc-documents
# Squad: DocumentIntelligenceSquad — agent pipeline defined in config/squads.yaml
...
class DocumentIntelligenceOrchestrator(BaseOrchestrator):
    """
    SBB Orchestrator for DocumentIntelligenceSquad.

    Extends ``BaseOrchestrator`` so zero trust fires before every squad execution.
    Subscribes to ``eoc-documents``. Agent execution order is fully declared in
    ...
    def start(self, squads_yaml_path: str) -> None:
        ...
        self._event_bus = K9EventBus(broker_url=broker, topic=_TOPIC, group_id="eoc-docs-grp")
        log.info("[DocumentIntelligenceOrchestrator] Listening on topic: %s", _TOPIC)
    ...
    def execute_flow(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        log.info("[DocumentIntelligenceOrchestrator] Processing document: event=%s", event_id)
        zt = self.apply_zero_trust(payload)
        if not zt["allowed"]:
            log.warning("[DocumentIntelligenceOrchestrator] Zero Trust DENIED event=%s reason=%s", event_id, zt["reason"])
